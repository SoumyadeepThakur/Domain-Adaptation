...
def transform_samples(X_source, Y_source, X_aug_list, Y_aug_list, X_target, Y_target):
	...
	ot_sinkhorn = ot.da.SinkhornTransport(reg_e=0.5, max_iter=200, norm="median", verbose=True)
	...
	for i in range(len(X_domain)):
		...
		for j in range(i+1,len(X_domain)):
			...
			ot_sinkhorn.fit(Xs=X_domain[i], ys=Y_domain[i], Xt=X_domain[j], yt=Y_domain[j])
...
def transform_samples_otda(X_source, Y_source, X_aug_list, Y_aug_list,  X_target, Y_target):
	...
	ot_sinkhorn_r = RegularizedSinkhornTransport(reg_e=0.5, max_iter=50, norm="median", verbose=True)
