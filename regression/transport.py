...
def transform_samples(X_source, Y_source, X_aug_list, Y_aug_list, X_target, Y_target):

	ot_sinkhorn = RegularizedSinkhornTransport(reg_e=0.5, alpha=10, max_iter=50, norm="median", verbose=True)
...
def transform_samples_otda(X_source, Y_source, X_aug_list, Y_aug_list,  X_target, Y_target):

	ot_sinkhorn_r = RegularizedSinkhornTransport(reg_e=0.5, alpha=10, max_iter=50, norm="median", verbose=True)
	...
	for i in range(len(X_domain)-1):
		...
		for j in range(i+1,len(X_domain)):
			...
			#if j==i+1: ot_sinkhorn_r.fit(Xs=X_domain[i], ys=Y_domain[i], Xt=X_domain[j], yt=Y_domain[j], iteration=0)
			#else: ot_sinkhorn_r.fit(Xs=X_domain[j-1], ys=Y_domain[j-1], Xt=X_domain[j], yt=Y_domain[j], prev_gamma=gamma, iteration=1)
			if j==i+1: ot_sinkhorn_r.fit(Xs=X_domain[i], Xt=X_domain[j], iteration=0)
			else: ot_sinkhorn_r.fit(Xs=X_domain[j-1], Xt=X_domain[j], prev_gamma=gamma, iteration=1)
			gamma = ot_sinkhorn_r.coupling_
			X_temp = ot_sinkhorn_r.transform(X_temp)
